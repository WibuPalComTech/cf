...
<div class="block full">
  <div class="block-title">
    <h2><strong>Semua Pesan</strong></h2>
    </div><?php 
	  // filter inputan id
    $id = $mysqli->escape_string($_POST['id']);

    $query =  $mysqli->query("DELETE FROM `pesan` WHERE `pesan`.`id_pesan` = '$id';");
    if($query){
     ?>
     <div class="alert alert-success alert-dismissable">
       <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
       <h4><i class="fa fa-info-circle"></i> Data Berhasil Dihapus !</h4>
       </div><?php } else { ?>
         <div class="alert alert-warning alert-dismissable">
           <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
           <h4><i class="fa fa-info-circle"></i> Data Gagal Dihapus, Silahkan Hubungi Admin !</h4>
         </div>
       <?php } ?>
       <div class="table-responsive">
		...
		// memanggil data dari tabel pesan yang ada pada database
            $pesan = $mysqli->query("select pesan.* from pesan");
