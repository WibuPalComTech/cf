...
<div class="block full">
  <div class="block-title">
    ...
    if (empty($password)){
      // meng-update data yang telah di inputkan jika tidak ada password yang di inputkan
      $query =  $mysqli->query("UPDATE `pengguna` SET `username` = '$username',`status` = '$status' WHERE `pengguna`.`kd_pengguna` = '$id';"); 
    } else { 
      // meng-update data yang telah di inputkan jika ada password yang di inputkan
      $query =  $mysqli->query("UPDATE `pengguna` SET `username` = '$username', `password` = md5('$password'),`status` = '$status' WHERE `pengguna`.`kd_pengguna` = '$id';"); 
    }
    if($query){
     ?>
